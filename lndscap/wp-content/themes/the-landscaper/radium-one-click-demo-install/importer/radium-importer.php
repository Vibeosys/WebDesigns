	...
	class Radium_Theme_Importer {
	    ...
	    public function demo_installer() {
			...
			if( !empty($this->imported_demos ) ) {
				...
				$button_text = esc_html__('Import Again', 'the-landscaper-wp');

			} else {

				$button_text = esc_html__('Import Demo Data', 'the-landscaper-wp');
	        ...
	        <div id="icon-tools" class="icon32"><br></div>
				...
				<form method="post">
					...
					<div style="background-color: #F5FAFD; margin:10px 0;padding: 20px; color: #0C518F; border: 2px solid #CAE0F3; clear:both; width:90%; line-height:18px;">
						<p class="tie_message_hint">Importing demo data (post, pages, placeholders, theme settings) is the easiest way to setup your theme.
						It will allow you to quickly edit everything instead of creating content from scratch. When you import the data following things will happen:</p>
						...
						<ul style="padding-left: 20px; list-style-position: inside; list-style-type: square;">
							...
							<li><strong>Need to manually import the .xml file? Follow <a href="https://codex.wordpress.org/Importing_Content#WordPress" target="_blank">these</a> steps</strong>
						</li></ul>

						<hr style="margin: 30px 0;">

						<h1>Start importing the demo data</h1>
						<ol style="padding-left: 20px;">
							<li>Install all required plugins</li>
							<li>Activate all required plugins</li>
							<li>Click the Import button <strong>once</strong> and <strong>wait</strong> till you see the 'Finished' message (might takes a couple of seconds).</li>
						</ol>
					</div>
					...
					<?php if( !empty($this->imported_demos) ) { ?>
						...
						<div style="background-color: #FAFFFB; margin:10px 0;padding: 5px 10px;color: #8AB38A;border: 2px solid #a1d3a2; clear:both; width:90%; line-height:18px;">
							<p><?php esc_html_e( 'Demo already imported', 'the-landscaper-wp' ); ?></p>
						</div>
					...
					<input name="reset" class="panel-save button-primary radium-import-start" type="submit" value="<?php echo esc_attr( $button_text ); ?>" />
	    ...
	    public function set_demo_theme_options( $file ) {
			...
			if ( file_exists( $file ) ) {
				...
				if ( !empty( $data ) || is_array( $data ) ) {
	      		...
	      		wp_die(
      				esc_html__( 'Theme options Import file could not be found. Please try again.', 'the-landscaper-wp' ),
      				'',
	    ...
	    function process_widget_import_file( $file ) {
	    	...
	    	if ( ! file_exists( $file ) ) {
	    		wp_die(
	    			esc_html__( 'Widget Import file could not be found. Please try again.', 'the-landscaper-wp' ),
	    			'',
	    			array( 'back_link' => true )
	    		);
	    	}
	    ...
	    public function import_widgets( $data ) {
	    	...
	    	foreach ( $data as $sidebar_id => $widgets ) {
	    		...
	    		if ( isset( $wp_registered_sidebars[$sidebar_id] ) ) {
	    			...
	    			$sidebar_message = esc_html__( 'Sidebar does not exist in theme (using Inactive)', 'the-landscaper-wp' );
	    		}
	    		...
	    		foreach ( $widgets as $widget_instance_id => $widget ) {
	    			...
	    			if ( ! $fail && ! isset( $available_widgets[$id_base] ) ) {
	    				...
	    				$widget_message = esc_html__( 'Site does not support widget', 'the-landscaper-wp' ); // explain why widget not imported
	    			}
	    			...
	    			if ( ! $fail && isset( $widget_instances[$id_base] ) ) {
	    				...
	    				foreach ( $single_widget_instances as $check_id => $check_widget ) {
	    					...
	    					if ( in_array( "$id_base-$check_id", $sidebar_widgets ) && (array) $widget == $check_widget ) {
	    						...
	    						$widget_message = esc_html__( 'Widget already exists', 'the-landscaper-wp' ); // explain why widget not imported
	    			...
	    			if ( ! $fail ) {
	    				...
	    				if ( $sidebar_available ) {
	    					...
	    					$widget_message = esc_html__( 'Imported', 'the-landscaper-wp' );
	    				} else {
	    					$widget_message_type = 'warning';
	    					$widget_message = esc_html__( 'Imported to Inactive', 'the-landscaper-wp' );
	    				}
	    			...
	    			$results[$sidebar_id]['widgets'][$widget_instance_id]['title'] = $widget->title ? $widget->title : esc_html__( 'No Title', 'the-landscaper-wp' ); // show "No Title" if widget instance is untitled
